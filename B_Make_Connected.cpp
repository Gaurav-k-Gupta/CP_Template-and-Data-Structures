...
bool check( vector<vector<char>>& a ){
    ...
    for(ll i = 0 ; i < n ; i++){
        for(ll j = 0 ; j < n ; j++){
            ...
            if( i && i < n-1 && a[i-1][j] == '#' && a[i+1][j] == '#' ) return false;
            if( j && j < n-1 && a[i][j-1] == '#' && a[i][j+1] == '#' ) return false;
        }
    }
    ...
    while( !q.empty() ){
        ...
        for(int k = 0 ; k < 4 ; k++){
            ...
            if( I >= 0 && I < n && J >= 0 && J < n && a[I][J] == '#' && !vis[I][J] ){
                q.push({I , J});
            }
        }
...
bool stepUp1( vector<vector<char>> a ){
    ll n = a.size();
...
bool stepUp2( vector<vector<char>> a ){
    ll n = a.size();
...
bool stepDown1( vector<vector<char>> a ){
    ll n = a.size();
...
bool stepDown2( vector<vector<char>> a ){
    ll n = a.size();
