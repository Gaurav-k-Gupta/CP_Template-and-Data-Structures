...
int main(){
    ...
    for( ll price = h[0] ; price <= x ; price++ ) prev[price] = s[0];


    for( ll i = 1 ; i < n ; i++ ){
        for( ll price = 0 ; price <= x ; price++ ){
            ll pick = 0;
            if( price >= h[i] ) pick = s[i] + prev[price - h[i]];

            ll nonPick = prev[price];

            curr[price] = max( pick , nonPick );
        }
