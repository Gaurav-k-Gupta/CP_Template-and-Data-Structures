...
ll result( ll d , ll L , ll R ){

    for(ll v = L ; v <= min( R , L + 1500) && ( v + d <= R ) ; v++){
        if( __gcd( v , d ) == 1 ){
            return v;
        }
    }
...
void solve(){
    ...
    for(ll v1 = L ; v1 <= min(R , v1 + 350) ; v1++){
        for(ll v2 = R ; v2 >= max( L , v2 - 350) ; v2--){
            if( v2 < v1 ){
                continue;
            }
            else if( v1 == v2 && v1 != 1 ) continue;
