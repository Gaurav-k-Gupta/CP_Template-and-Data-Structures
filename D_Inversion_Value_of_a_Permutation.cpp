...
bool validPerm( int i , int j , ll k , vector<vector<vll>>& dp ){
    ...
    dp[i][j][k] = validPerm( i-1 , j , k - cnt , dp ) | validPerm( i-1 , i , k - cnt , dp );
...
void solve(){
    ...
    // dp( i , j , k ) => whether a perm exist or not with i being the current index , j the starting index of the next increasing segment, k current inversion count remaining 


    vector<vector<vll>> dp( n , vector<vll>( n+1 , vll( k+1 , -1 )));

    bool res = validPerm( n-1 , n , k , dp );
