...
void solve(){
    ...
    ll currCap = -1 , currCon = -1;

    ll cost = 0;
    while( currCon < n-1 ){
        currCon++;
        if( currCap == -1 ) cost += b*c[currCon];
        else cost += b*( c[currCon] - c[currCap] );

        ll capRel = a*c[currCap+1];
        if( currCap != -1 ) capRel -= a*c[currCap];

        ll benefit = b*(capRel/a)*(n-currCon-1);

        while( currCap < currCon && capRel <= benefit ){
            currCap++;
            cost += capRel;
            capRel = a*(c[currCap+1]-c[currCap]);
            benefit = b*(capRel/a)*(n-currCon-1);
        }
