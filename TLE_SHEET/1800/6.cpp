...
void solve(){
    ...
    for(int i = 1 ; i < n ; i++){
        for(int j = 0 ; j < 3 ; j++){
            ll pick = 0;
            if( s1[j] == a[i] ) pick = dp1[i-1][(j+2)%3];
            ll nonPick = dp1[i-1][j];
            dp1[i][j] = ( pick + nonPick ) % mod; 
        }
    }



    vector<vll> dp2(n , vll(3 , 0));
    dp2[0][0] = 1;
    if( !a[0] ) dp2[0][1] = 1;
    vll s2 = { 1 , 0 , 1 };
    for(int i = 1 ; i < n ; i++){
        for(int j = 0 ; j < 3 ; j++){
            ll pick = 0;
            if( s2[j] == a[i] ) pick = dp2[i-1][(j+2)%3];
            ll nonPick = dp2[i-1][j];
            dp2[i][j] = ( pick + nonPick ) % mod; 
        }
    }


    vector<vll> dp3(n , vll(3 , 0));
    ...
    for(int i = 1 ; i < n ; i++){
        for(int j = 0 ; j < 3 ; j++){
            ll pick = 0;
            if( s3[j] == a[i] ) pick = dp3[i-1][(j+2)%3];
            ll nonPick = dp3[i-1][j];
            dp3[i][j] = ( pick + nonPick ) % mod; 
        }
    }



    vector<vll> dp4(n , vll(3 , 0));
    ...
    for(int i = 1 ; i < n ; i++){
        for(int j = 0 ; j < 3 ; j++){
            ll pick = 0;
            if( s4[j] == a[i] ) pick = dp4[i-1][(j+2)%3];
            ll nonPick = dp4[i-1][j];
            dp4[i][j] = ( pick + nonPick ) % mod; 
        }
    }
    ...
    ll cnt = 0;
    for(ll j = 0 ; j < 3 ; j++){
        cnt = ( cnt + dp1[n-1][j] ) % mod;
        cnt = ( cnt + dp2[n-1][j] ) % mod;
        cnt = ( cnt + dp3[n-1][j] ) % mod;
        cnt = ( cnt + dp4[n-1][j] ) % mod;
    }
