...
void solve(){
    ...
    vector<vector<ppll>> seg(m+1);
    for(int i = 0 ; i < n ; i++){
        ll li , ri , p , q;
        cin>>li>>ri>>p>>q;

        seg[ri].push_back({ li , { p , q }});
    }
    ...
    for(auto & it : seg){
        for(auto & x : it){
            ll p = x.second.first;
            ll q = x.second.second;

            ll prob = ( ( q - p ) * ( modPow(q , mod-2) % mod ) ) % mod;
    ...
    for(ll i = 1 ; i <= m ; i++){
        ...
        for(auto & x : seg[i]){
            ...
            // dp[i] += dp[li-1] * ( p / q ) * ( q / ( q - p ) )
            
            ll inc = (( ( dp[li-1] * p ) % mod ) * q ) % mod;
            inc = ( inc * modPow(q , mod-2) ) % mod;
            inc = ( inc * modPow(q-p , mod-2) ) % mod;
