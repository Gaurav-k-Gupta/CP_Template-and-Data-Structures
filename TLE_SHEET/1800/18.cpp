...
void solve(){
    ...
    sort( edges.begin() , edges.end() , [&](pair<ll,pll> & a , pair<ll,pll> & b){   
    ...
    ll closest_val = INF;
    for(ll i = 0 ; i < m ; i++){
        if( ds.Union(edges[i].second.first , edges[i].second.second) ){
            cnt += max( 0LL , edges[i].first - k );
            maxi = max( maxi , edges[i].first ); 
        }
        closest_val = min( closest_val , abs(edges[i].first - k) ); 
    }

    // cout<<maxi<<endl;

    if( maxi >= k ) cout<<cnt<<endl;
    else{
