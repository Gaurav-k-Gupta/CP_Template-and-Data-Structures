...
void solve(){
    ...
    vll preSum(n,0) , preXor(n,0);
    preSum[0] = preXor[0] = a[0];
    ...
    for(ll i = 0 ; i < n ; i++){
        ...
        ll val = ( preSum[r] - ( i ? preSum[i-1] : 0 ) ) - ( preXor[r] ^ ( i ? preXor[i-1] : 0 ) );

        if( val != maxi ) continue;

        while( l <= r ){
            ll mid = ( l + r ) / 2;
            val = ( preSum[mid] - ( i ? preSum[i-1] : 0 ) ) - ( preXor[mid] ^ ( i ? preXor[i-1] : 0 ) );
            if( val == maxi ) r = mid - 1;
            else l = mid + 1;
        }
