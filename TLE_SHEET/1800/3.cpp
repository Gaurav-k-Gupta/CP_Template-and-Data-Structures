...
void solve(){
    ...
    for(ll i = 1 ; i < m ; i++){
        for( ll hp = 0 ; hp <= sum_h ; hp++ ){
            ll pick = -1 , nonPick = -1;
            if( hp - h[i] >= 0 ){
                ll val = dp[i-1][hp-h[i]];
                if( val + x >= x && val + x >= c[i] ) pick = val + x - c[i];
            }

            if( dp[i-1][hp] != -1 ) nonPick = dp[i-1][hp] + x;

            dp[i][hp] = max( pick , nonPick );
        }
    }
