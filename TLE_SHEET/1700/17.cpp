...
bool check( vll & a , vector<vll>& divisors , ll diff , ll m ){
    ll n = a.size();
    unordered_map<ll , ll> st;
    ll i = 0;
    ll cnt = 0;
    while( i < n && a[i] - a[0] <= diff ){
    ...
    while( i < n ){
        ...
        // remove from prev , get to the max i 
        for( auto & x : divisors[a[prev]] ){
            st[x]--;
            if( !st[x] ) cnt--;
        } 
        prev++;
        while( i < n && a[i] - a[prev] <= diff ){
            for(auto & x : divisors[a[i]]){
                if( x <= m ){
                    if(!st[x]) cnt++;
                    st[x]++;
...
void solve(vector<vll> & divisors){
    ll n , m;
    cin>>n>>m;
    ...
    while( low <= high ){
        ll mid = ( low + high ) / 2;
        if( check(a , divisors , mid , m ) ) high = mid - 1;
        else low = mid + 1;
    }

    if( low <= a[n-1] - a[0] ) cout<<low<<endl;
    else cout<<"-1"<<endl;
