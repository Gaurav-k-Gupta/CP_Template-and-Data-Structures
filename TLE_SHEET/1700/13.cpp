...
ll help( ll i , ll val , vll & a , ll k , vector<vll>& dp ){

    if( i == a.size() ){
        if( val == 64 ) return 0LL;
    ...
    ll pick = 0;
    if( val == 64 ) pick = help( i+1 , a[i] , a , k , dp );
    else pick = help(i+1 , val & a[i] , a , k , dp );

    ll nonPick = help(i+1 , val , a , k , dp );

    return dp[i][val] = ( pick + nonPick ) % mod;
}
...
void solve(){
    ...
    vector<vll> dp( n , vll(65 , -1));

    ll ans = help(0 , 64 , a , k , dp);
    cout<<ans<<endl;
}
