...
void solve(){
    ...
    sort(seg.begin() , seg.end() , [&](ppll & a , ppll & b){
        return a.second.first < b.second.first;
    });

    vll res(n);

    ll maxEnd = seg[0].second.second;
    res[seg[0].first] = 1;
    bool flag = false;
    for(int i = 1 ; i < n ; i++){

        if( seg[i].second.first > maxEnd ){
            flag = true;
        }

        if( flag ) res[seg[i].first] = 2;
        else res[seg[i].first] = 1;

        maxEnd = max( maxEnd , seg[i].second.second );
    }
