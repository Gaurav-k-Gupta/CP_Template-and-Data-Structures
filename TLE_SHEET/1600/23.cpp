...
ll cntAns( ll i , ll j , vll & a , vector<vll>& dp1 , vector<vll>& dp2 ){
    if( i == 0 && j <= 1 ) return 1LL;

    if( dp2[i][j] != -1 ) return dp2[i][j];

    ll nonPick = 0;
    if( dp1[i-1][j] == dp1[i][j] ) nonPick = cntAns( i-1 , j , a , dp1 , dp2 );
    ll pick = 0;
    if( j > 0 && dp1[i-1][j-1] + a[i] == dp1[i][j] ) pick = cntAns(i-1 , j-1 , a , dp1 , dp2 );

    return dp2[i][j] = ( pick + nonPick ) % mod;
}



ll cntMax( ll idx , ll k , vll & a  , vector<vll>& dp ){
    if( idx == a.size() || !k ) return 0LL;

    if( dp[idx][k] != -1 ) return dp[idx][k];

    ll pick = a[idx] + cntMax( idx + 1 , k - 1 , a , dp );
    ll nonPick = a[idx] + cntMax( idx+1 , k , a , dp );

    return dp[idx][k] = max( pick , nonPick );
}
...
void solve(){
    ...
    vector<vll> dp2(n , vll( k+1 , -1 ));
    dp2[0][1] = 1;
    dp2[0][0] = 1;

    ll ans = cntAns(n-1 , k , a , dp , dp2 );
    cout<<ans<<endl;
