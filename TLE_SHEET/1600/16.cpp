...
ll dfs(ll node , vector<vll>& adj , vll & vis ){
    
    vis[node] = 1;
    // cout<<node<<endl;
    for( auto it : adj[node] ){
        if(!vis[it]) return (dfs(it , adj , vis) + 1);
    }
...
void help( int i , int j , vll & a ){
    ...
    // cout<<p1<<" "<<p2<<" "<<left_idx<<" "<<right_idx<<endl;
