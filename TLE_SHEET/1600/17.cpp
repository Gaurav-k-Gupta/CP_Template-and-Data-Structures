...
void minSteps(ll v , ll steps ){
    if( v > 1000 ) return;

    if( dp1[v] < steps ) return;
...
ll maxCoins( ll idx , ll k , vll & b , vll & c , vector<vll>& dp2){
    if( idx == b.size() ) return 0LL;

    if( dp2[idx][k] != -1 ) return dp2[idx][k];

    ll steps = dp1[b[idx]];
    ll pick = 0;
    if( k >= steps )  pick = c[idx] + maxCoins(idx+1 , k - steps , b , c , dp2);
    ll nonPick = maxCoins(idx+1 , k , b , c , dp2);

    dp2[idx][k] = max( pick , nonPick );
    return dp2[idx][k];
}
...
void solve(){
    ...
    vector<vll> dp2(n , vll(k_max+1 , -1));
    ll ans = maxCoins(0 , k_max , b , c , dp2);
    cout<<ans<<endl;
}
...
int main(){
    ...
    for(ll i = 1 ; i <= 1000 ; i++){
        for(ll j = 1 ; j <= i ; j++){
            ll nxt = i + i/j;
            if( nxt <= 1000 ) dp1[nxt] = min( dp1[nxt] , 1 + dp1[i]);
        }
    }
