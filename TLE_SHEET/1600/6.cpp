...
bool checkSeg( vector<pll>& seg , vll& que , ll k ){
    vll ones( que.begin() , que.begin()+k );
    ...
    for( auto it : seg ){
        ...
        ll low = lower_bound(ones.begin() , ones.end() , l) - ones.begin();
        ll high = lower_bound(ones.begin() , ones.end() , r) - ones.begin();

        if( high < k && ones[high] != r ) high--;
        else if( high == k ) high--;
...
void dfs( ll node , vector<set<ll>>& adj , bool & check , vll & vis){
    vis[node] = 1;

    for(auto it : adj[node]){
        if( !vis[it] ) dfs(it , adj , check , vis);
    }

    if( adj[node].size() == 1 ) check = true;
    return;
}
