...
bool check(ll idx , vll & b , vll & dp , vector<vll>& adj){
    if( idx == b.size() ) return true;
    if( idx > b.size() ) return false;

    if( dp[idx] != -1 ) return dp[idx];

    bool pick = check(idx + b[idx] + 1 , b , dp , adj);
    bool nonPick = false;
    for(auto it : adj[idx]){
        nonPick = nonPick | check(it+1 , b , dp , adj);
    }

    return dp[idx] = (pick | nonPick);
}
...
void solve(){
    ...
    for(int i = n-1 ; i >= 0 ; i--){
        ll idx = i - b[i];
        if( idx >= 0 ) adj[idx].push_back(i);
    }

    vll dp(n , -1);

    bool ans = check(0 , b , dp , adj);
    if( ans ) yes;
    else no;
