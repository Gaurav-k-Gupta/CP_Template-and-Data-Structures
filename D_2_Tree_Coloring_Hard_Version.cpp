...
void solve(){
    ...
    ll mf = *max_element( freq.begin() , freq.end() );

    unordered_set<ll> st;
    for(ll i = 0 ; i <= n ; i++){
        if( freq[i] == mf ) st.insert( i ); 
    }

    ll res = mf;

    if( check(0 , -1 , adj , d , st , mf ) ) res++;

    cout<<res<<endl;

    vector<unordered_set<ll>> col( n+1 );
    for(ll i = 0 ; i <= n ; i++){
        ll f = freq[i];
        f++;
        f = min( f , res );
        ...
        cout<<i<<" :: "<<col[i].size()<<endl;
    }

    vll co( n , -1 );
    co[0] = 0;
    dfs2(0 , -1 , adj , co , d , col);

    vector<vll> ans( res );
    for(ll i = 0 ; i < n ; i++){
        ans[ co[i] ].push_back( i );
    }
