...
ll cntNodes( ll u , ll par , vector<vll>& adj , vll & res ){
    ll n = adj.size();
    ll cnt = 1;
    for(auto & it : adj[u]){
        if( it == par ) continue;
        ll c = cntNodes( it , u , adj , res );
        if( c ) res.pb(c*(n-c));
        cnt += c;
    }
...
void solve(){
    ...
    for(ll i = 0 ; i < n ; i++){
        if( !(i & 1) ){
            val = max( val , (-2*a[i] + i + mini2));
            mini1 = min( mini1 , i + 2*a[i] );
            sum += a[i];
        }
        else{
            val = max( val , (2*a[i] + i - mini1 ));
            mini2 = max(mini2 , -i + 2*a[i]);
            sum -= a[i];
        }
    }

    cost = ( n & 1 ) ? n - 2 - 1 : n - 1 - 1;

    val = max( val , cost );

    cost = ( n & 1 ) ? n - 1 - 0 : n - 2 - 0;
