...
void solve(){
    ...
    if( x == 1 && y == 1 ){
        if( (rk == 0 && ck == 0) || (rk == n && ck == n) || (rk == 0 && ck == n) || (rk == n && ck == 0)){
            cout<<1<<endl;
            return;
        }
    }
    ...
    if( x == 0 ){
        ...
        ll cox , coy;
        if( rd > rk && cd > ck ){ cox = 0; coy = 0; }
        else if( rd > rk && cd < ck ){ cox = 0; coy = n; }
        else if( rd < rk && cd > ck ){ cox = n; coy = 0; }
        else cox = n , coy = n;

        ll dx = abs( rk - cox );
        ll dy = abs( ck - coy );
        
        ll res = dx + dy + max( x - dy , y - dx );
        cout<<res<<endl;
    }
