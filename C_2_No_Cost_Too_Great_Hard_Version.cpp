...
void solve( vector<vll>& p_f ){
    
    ll n;
    cin>>n;

    vector<pll> ab(n);
    unordered_map<ll,ll> f_cnt;
    bool zero = false;
    // ll lcmm = 1;
    for(ll i = 0; i < n ; i++){
        cin>>ab[i].second;

        // lcmm = lcm( lcmm , ab[i].second );

        for(auto & it : p_f[ab[i].second]){
            f_cnt[it]++;
            if( f_cnt[it] >= 2 ) zero = 1;
        }
    ...
    // cout<<c<<endl;
        ...
        for(ll i = 1 ; i < n ; i++){
            for(auto & v : p_f[ab[i].second]){
                ll ai = ab[0].second;

                ll c = (( (ai / v) + 1 ) * v) - ai;

                ans = min( ans , c * ab[0].first );
            }
        }
...
int main(){
    ...
    // vll p = primes( 2e5 );
    vector<vll> p_f = prime_factorization( 2e5 );
