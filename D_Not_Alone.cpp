...
ll cost3( ll a , ll b , ll c ){
    ll mini = 1e9;

    mini = min( mini , cost2(a,b) + cost2(a,c));
    mini = min( mini , cost2(b,a) + cost2(b,c));
    mini = min( mini , cost2(c,a) + cost2(c,b));
...
ll cost( vll & a ){
    ...
    for(ll i = 2 ; i <= n ; i++){
        ll si2 = dp[i-2] + cost2( a[i-1] , a[i-2] );

        ll si3 = 1e12;
        if( i >= 3 ) si3 = dp[i-3] + cost3( a[i-1] , a[i-2] , a[i-3] );
