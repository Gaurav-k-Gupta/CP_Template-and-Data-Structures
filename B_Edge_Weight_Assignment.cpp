...
// odd , even
pair<ll,ll> dfs(ll u , ll par , vector<vector<ll>>& adj , bool & check){
    ll od = 0 , ev = 0;

    for(auto it : adj[u]){
        if( it == par ) continue;
        pair<ll,ll> res = dfs( it , u , adj , check );
        od += res.first;
        ev += res.second;


        // cout<<u<<" "<<it<<" "<<od<<" "<<ev<<endl; 
    }
...
void solve(){
    ...
    bool check = false;
    pair<ll,ll> res = dfs(0 , -1 , adj , check);

    // cout<<check<<endl;

    if( adj[0].size() == 1 ){
        if( res.second ) check = true;
    }
